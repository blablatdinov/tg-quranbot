...
async def test(register_service, ayat_repository_mock, user_repository_mock):
    ...
    assert got == [
        TextAnswer(chat_id=231, message='start message_handlers'),
        TextAnswer(chat_id=231, message=str(ayat_repository_mock.storage[0])),
    ]
...
async def test_already_registered_user(
    ...
    assert got == TextAnswer(chat_id=444, message='Вы уже зарегистрированы')
    # TODO: check, that event about user action sended in queue
...
async def test_inactive_user(register_service, user_repository_with_registered_inactive_user):
    ...
    assert got == TextAnswer(chat_id=444, message='Рады видеть вас снова, вы продолжите с дня 15')
    # TODO: check, that event about user action sended in queue
...
async def test_with_referrer(
    ...
    assert got == [
        TextAnswer(chat_id=222, message='start message_handlers'),
        TextAnswer(chat_id=222, message=str(ayat_repository_mock.storage[0])),
        TextAnswer(chat_id=444, message='По вашей реферральной ссылке произошла регистрация'),
    ]
    # TODO: check, that event about user action sended in queue
